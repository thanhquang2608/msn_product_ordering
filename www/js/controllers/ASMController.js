...
app.controller('ASMController', function ($scope, $rootScope, $timeout, $stateParams, $state, $modal, $log, $interval, $q, $stickyState, $translate,
    ...
    $rootScope.loadOrderInMonth = function () {
        ...
        if ($state.current.name != 'home.asm') {
            console.log('click');
            if ($state.current.name == 'home.asm-order-detail') {
                var data = {};
                data.ProvinceId = $scope.order.ProvinceId;
                $state.go('home.asm', { DealerId: $scope.order.DealerId, Data: data, Year: $stateParams.Year, Month: $stateParams.Month }, { reload: true });
            }
            else {
                var dealerId = null;
            ...
            console.log('done');
            if ($scope.functions.indexOf($scope.roleFunction.ASM.LIST_ORDER) > -1) {
                $scope.lock = true;
    ...
    $scope.currentLevel = USER_LEVELS.ASM;
    ...
    $scope.orderPage = function () {
        ...
        $state.go('home.asm-order', { DealerId: $scope.selected.Dealer.DealerId, ProvinceId: $scope.selected.Province.ProvinceId }, { reload: true });
    }

    $scope.orderDetail = function (orderId) {
        $state.go('tabs.asm-list-order-detail', { OrderId: orderId });
    }
    /////////// END SALE HOME
    ...
    $scope.previewOrder = function (isValid) {
        ...
        $state.go('tabs.asm-order-review', { Data: data }).then(function () {
            $scope.submited = false;
            $scope.sumFlag = false;
        })
    }
    ...
    $scope.duplicateOrder = function (order) {
        ...
        $stickyState.reset('tabs.asm-order');
        $state.go('tabs.asm-order', { }, {});
    }
    /////// END ORDER DETAIL
    ////////// BEGIN SALE INFO
