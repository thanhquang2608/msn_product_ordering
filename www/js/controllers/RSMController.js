...
app.controller('RSMController', function ($scope, $rootScope, $timeout, $stateParams, $state, $modal, $log, $interval, $q, $stickyState, $translate,
    ...
    $rootScope.loadOrderInMonth = function () {
        ...
        if ($state.current.name != 'home.rsm') {
            console.log('click');
            if ($state.current.name == 'home.rsm-order-detail') {
                var data = {};
                data.ProvinceId = $scope.order.ProvinceId;
                $state.go('home.rsm', { DealerId: $scope.order.DealerId, Data: data, Year: $stateParams.Year, Month: $stateParams.Month }, { reload: true });
            }
            else {
                var dealerId = null;
            ...
            console.log('done');
            if ($scope.functions.indexOf($scope.roleFunction.RSM.LIST_ORDER) > -1) {
                $scope.lock = true;
    ...
    $scope.currentLevel = USER_LEVELS.RSM;
    ...
    $scope.orderPage = function () {
        ...
        $state.go('home.rsm-order', { DealerId: $scope.selected.Dealer.DealerId, ProvinceId: $scope.selected.Province.ProvinceId }, { reload: true });
    }

    $scope.orderDetail = function (orderId) {
        $state.go('tabs.rsm-list-order-detail', { OrderId: orderId });
    }
    /////////// END SALE HOME
    ...
    $scope.duplicateOrder = function (order) {
        ...
        $stickyState.reset('tabs.rsm-order');
        $state.go('tabs.rsm-order', {}, { reload: true });
    }
    /////// END ORDER DETAIL
    ////////// BEGIN SALE INFO
