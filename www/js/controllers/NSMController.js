...
app.controller('NSMController', function ($scope, $rootScope, $timeout, $stateParams, $state, $modal, $log, $interval, $q, $stickyState, $translate,
    ...
    $rootScope.loadOrderInMonth = function () {
        ...
        if ($state.current.name != 'home.nsm') {
            console.log('click');
            if ($state.current.name == 'home.nsm-order-detail') {
                var data = {};
                data.ProvinceId = $scope.order.ProvinceId;
                $state.go('home.nsm', { DealerId: $scope.order.DealerId, Data: data, Year: $stateParams.Year, Month: $stateParams.Month }, { reload: true });
            }
            else {
                var dealerId = null;
            ...
            console.log('done');
            if ($scope.functions.indexOf($scope.roleFunction.NSM.LIST_ORDER) > -1) {
                $scope.lock = true;
    ...
    $scope.currentLevel = USER_LEVELS.NSM;
    ...
    $scope.orderPage = function () {
        ...
        $state.go('home.nsm-order', { DealerId: $scope.selected.Dealer.DealerId, ProvinceId: $scope.selected.Province.ProvinceId }, { reload: true });
    }

    $scope.orderDetail = function (orderId) {
        $state.go('tabs.nsm-list-order-detail', { OrderId: orderId });
    }
    /////////// END SALE HOME
    ...
    $scope.cancelOrder = function (order) {
        ...
        modalConfirm.result.then(function (result) {
            ...
            CommonService.removeOrder(order.OrderId, $scope.currentRole, $scope.currentLevel, result).then(function (data) {
                $scope.modalProgress.dismiss('close');
                document.body.style.cursor = 'auto';
                $state.go('tabs.nsm-list', { DealerId: $scope.order.DealerId }, { reload: true }).then(function () {
                });
            }, function (err) {
                $scope.modalProgress.dismiss('close');
                document.body.style.cursor = 'auto';
    ...
    $scope.duplicateOrder = function (order) {
        ...
        $stickyState.reset('tabs.nsm-order');
        $state.go('tabs.nsm-order', {}, { reload: true });
    }
    /////// END ORDER DETAIL
    ////////// BEGIN SALE INFO
